...
COLORS_DARK = {'Štandardný výkon':(0.15,0.55,0.85,1),'T.V. (Turnus voľno)':(0.28,0.32,0.38,1),'Ranná':(0.15,0.55,0.85,1),'Poobedná':(0.85,0.55,0.15,1),'Nočná':(0.32,0.32,0.82,1),'Voľno':(0.22,0.60,0.26,1)}
COLORS_LIGHT = {'Štandardný výkon':(0.12,0.42,0.75,1),'T.V. (Turnus voľno)':(0.78,0.78,0.80,1),'Ranná':(0.12,0.42,0.75,1),'Poobedná':(0.90,0.45,0.05,1),'Nočná':(0.28,0.28,0.72,1),'Voľno':(0.22,0.62,0.28,1)}
...
class ShiftCalculator:
    ...
    def calculate_shift(self,ds,ss,es,plan="",pcs="",pce="",pnps="",pnpe="",st="",mo="",ot=""):
        ...
        elif ptt and st not in ["Štandardný výkon","T.V. (Turnus voľno)","4181-Náhr.za pr.poh.v p.",""]: has=True; tot=ptt.hour*60+ptt.minute
...
class FilePickerPopup(Popup):
    def __init__(self,start_dirs,callback,**kw):
        super().__init__(**kw); self.callback=callback; self.title="Vyber súbor"; self.size_hint=(0.92,0.85)
        box=BoxLayout(orientation='vertical',spacing=dp(8),padding=dp(10)); scroll=ScrollView(); grid=GridLayout(cols=1,spacing=dp(4),size_hint_y=None); grid.bind(minimum_height=grid.setter('height')); files=[]
        for d in start_dirs:
            if os.path.exists(d):
                try:
...
class ZSSKApp(App):
    ...
    def open_editor(self,uid=None):
        ...
        inp_date=TextInput(text=edit.get('date',''),multiline=False,size_hint_y=None,height=dp(40),background_color=self.theme['input_bg'],foreground_color=self.theme['input_fg']); add_row('Dátum YYYY-MM-DD',inp_date)
    ...
    def open_payroll(self,*a):
        ...
        wh=t.get('worked',0)/60.0; nh=t.get('night',0)/60.0; sh=t.get('sat',0)/60.0; suh=t.get('sun',0)/60.0; hh=t.get('hol',0)/60.0
        sb=wh*hod; sn=nh*2.10; ss=sh*2.63; su=suh*5.26; shol=hh*13.88; hr=sb+sn+ss+su+shol+t.get('other',0)+65; ci=hr*0.73; kv=ci+t.get('meal',0)
        pop=Popup(title=f'Výplata {self.month_key()}',size_hint=(0.92,None),height=dp(480)); box=BoxLayout(orientation='vertical',padding=dp(12),spacing=dp(4))
        for txt in [f"Norma {self.get_auto_norm_str()} = {calendar.monthrange(self.cur_year,self.cur_month)[1]} dní × 5:08",f"Odprac: {wh:.1f}h × {hod:.2f} = {sb:.2f}€",f"Nočné: {sn:.2f}€",f"So+Ne: {(ss+su):.2f}€",f"Sviatok: {shol:.2f}€",f"Iné+65: {t.get('other',0)+65:.2f}€","---",f"HRUBÁ: {hr:.2f}€",f"ČISTÁ ~73%: {ci:.2f}€",f"Stravné: {t.get('meal',0):.2f}€",f"SPOLU: {kv:.2f}€",f"Saldo k norme: {saldo//60:+d}:{(abs(saldo)%60):02d}"]:
    ...
    def show_info(self,msg):
        p=Popup(title='Info',size_hint=(0.86,None),height=dp(300)); b=BoxLayout(orientation='vertical',padding=dp(12),spacing=dp(8)); b.add_widget(Label(text=msg,halign='center',color=self.theme['text'],font_size='12sp')); btn=Button(text='OK',size_hint_y=None,height=dp(42),background_normal='',background_color=self.theme['btn_settings']); btn.bind(on_press=p.dismiss); b.add_widget(btn); p.content=b; p.open()
