import json, os, datetime, calendar, uuid, csv, sys
...
def format_date_display(iso_date):
    try: y,m,d=iso_date.split('-'); return f"{int(d)}-{int(m)}-{y}"
...
class TextImportPopup(Popup):
    def __init__(self, callback, **kw):
        ...
        b_paste.bind(on_press=lambda *_: setattr(self.inp,'text',Clipboard.paste())); b_clear.bind(on_press=lambda *_: setattr(self.inp,'text','')); row.add_widget(b_paste); row.add_widget(b_clear); root.add_widget(row)
        btns=BoxLayout(size_hint_y=None,height=dp(48),spacing=dp(8)); b_cancel=Button(text='Zrušiť',background_normal='',background_color=(0.4,0.4,0.42,1)); b_imp=Button(text='📥 Importovať',background_normal='',background_color=(0.12,0.55,0.25,1),bold=True)
        b_cancel.bind(on_press=self.dismiss); b_imp.bind(on_press=self.do_import); btns.add_widget(b_cancel); btns.add_widget(b_imp); root.add_widget(btns)
        self.content=root
...
class FilePickerPopup(Popup):
    ...
    def build_ui(self):
        ...
        b_text=Button(text='⚠ Nevidím Download? Použi 📋 Import z textu',size_hint_y=None,height=dp(42),background_normal='',background_color=(0.68,0.42,0.12,1),font_size='11sp')
...
class ZSSKApp(App):
    def build(self):
        ...
        self.btn_select=Button(text='✓ Výber',size_hint_x=None,width=dp(88),background_normal='',background_color=self.theme['btn_settings'],font_size='12sp'); self.btn_select.bind(on_press=self.toggle_select_mode)
        b_add=Button(text='+ Pridať zmenu',background_normal='',background_color=self.theme['btn_primary'],font_size='14sp',bold=True); b_pay=Button(text='💰 Výplata',size_hint_x=None,width=dp(92),background_normal='',background_color=self.theme['btn_stats'],font_size='11sp')
        b_add.bind(on_press=lambda x:self.open_editor()); b_pay.bind(on_press=self.open_payroll); bottom.add_widget(self.btn_select); bottom.add_widget(b_add); bottom.add_widget(b_pay); root.add_widget(bottom)
        self.bulk_bar=BoxLayout(size_hint_y=None,height=dp(0),spacing=dp(6),padding=dp(3),opacity=0)
        self.btn_bulk_del=Button(text='🗑 Zmazať označené (0)',background_normal='',background_color=self.theme['btn_danger'],font_size='12sp'); self.btn_bulk_month=Button(text='🗑 Vymazať mesiac',size_hint_x=None,width=dp(140),background_normal='',background_color=self.theme['btn_danger'],font_size='11sp'); self.btn_bulk_cancel=Button(text='Zrušiť výber',size_hint_x=None,width=dp(100),background_normal='',background_color=self.theme['btn_settings'],font_size='11sp')
        self.btn_bulk_del.bind(on_press=self.delete_selected_confirm); self.btn_bulk_month.bind(on_press=self.delete_month_confirm); self.btn_bulk_cancel.bind(on_press=self.toggle_select_mode); self.bulk_bar.add_widget(self.btn_bulk_del); self.bulk_bar.add_widget(self.btn_bulk_month); self.bulk_bar.add_widget(self.btn_bulk_cancel); root.add_widget(self.bulk_bar)
    ...
    def apply_calculator_rates(self):
        try:
            rates = {
                "meal_a": float(str(self.employee.get('meal_a', '9.30')).replace(',', '.')),
                "meal_b": float(str(self.employee.get('meal_b', '13.80')).replace(',', '.')),
                "meal_c": float(str(self.employee.get('meal_c', '21.00')).replace(',', '.'))
            }
            self.calculator.update_rates(rates)
        except Exception as e:
            print("Chyba nastavenia stravného:", e)

    def check_bundled_data(self):
        try:
            bundled_files=find_bundled_data()
            if not bundled_files: return
            current=self.load_data()
            for bf in bundled_files:
                bdata=load_bundled_json_file(bf)
                if not bdata: continue
                shifts=bdata.get('shifts_data',[])
                if not shifts: continue
                if len(current)==0 or len(shifts) > len(current):
                    if len(current)==0:
                        self.save_data(shifts)
                        if bdata.get('turnus'):
                            self.save_turnus_file(bdata['turnus'])
                        self.show_info(f"Načítané vstavané dáta z APK:\n{os.path.basename(bf)}\n{len(shifts)} záznamov")
                        return
        except Exception as e:
            print(f"check_bundled_data chyba: {e}")

    def load_bundled_forced(self):
        bundled_files=find_bundled_data()
        if not bundled_files:
            self.show_info("V APK sa nenašiel žiadny data.json ani bbbbbb.json\nUisti sa že data.json je v repozitári vedľa main.py")
            return
        best=None; best_len=0
        for bf in bundled_files:
            bd=load_bundled_json_file(bf)
            if bd and len(bd.get('shifts_data',[]))>best_len:
                best=bd; best_len=len(bd.get('shifts_data',[]))
        if not best:
            self.show_info("Bundled súbory sú prázdne")
            return
        pop=Popup(title='Načítať vstavané dáta z APK?',size_hint=(0.90,None),height=dp(340),separator_color=self.theme['accent'])
        box=BoxLayout(orientation='vertical',padding=dp(14),spacing=dp(10))
        box.add_widget(Label(text=f"Našiel sa súbor:\n{best.get('source')}\n{best_len} záznamov\n\nChceš PREPÍSAŤ aktuálne dáta v aplikácii týmito vstavanými?",halign='center',color=self.theme['text'],font_size='11sp'))
        row=BoxLayout(size_hint_y=None,height=dp(48),spacing=dp(8))
        b1=Button(text='Zrušiť',background_normal='',background_color=self.theme['btn_settings'])
        b2=Button(text=f'Áno, načítať {best_len}',background_normal='',background_color=self.theme['btn_save'],bold=True)
        b1.bind(on_press=pop.dismiss)
        def do_load(*_):
            self.save_data(best.get('shifts_data',[]))
            if best.get('turnus'):
                self.save_turnus_file(best['turnus'])
            self.refresh()
            pop.dismiss()
            self.show_info(f"Načítaných {best_len} záznamov z APK")
        b2.bind(on_press=do_load)
        row.add_widget(b1); row.add_widget(b2); box.add_widget(row); pop.content=box; pop.open()
    ...
    def refresh(self):
        ...
        for s in month_data:
            ...
            if calc:
                ...
                else: totals['worked']+=p(calc.get('total_hours','0:00')) if calc.get('total_hours')!='-' else 0
            ...
            uid=s.get('uid'); typ=s.get('shift_type') or 'Štandardný výkon'; base=theme_colors.get(typ, (0.3,0.3,0.35,1))
            ...
            row_date=format_date_with_weekday(s.get('date',''), s.get('turnus',''))
            if self.select_mode:
                row=BoxLayout(orientation='horizontal',size_hint_y=None,height=dp(72),spacing=dp(4)); is_sel=uid in self.selected_uids
                ...
                main=Button(size_hint_x=1,background_normal='',background_color=base,color=(1,1,1,1),halign='left',valign='middle',padding=(dp(8),dp(4)),markup=True)
                main.text=f"[b]{row_date}[/b] {typ[:18]}"; main.bind(size=lambda inst,*a: setattr(inst,'text_size',(inst.width-dp(16),None))); main.bind(on_press=lambda inst,u=uid: self.toggle_one(u)); row.add_widget(cb); row.add_widget(main); self.grid.add_widget(row)
            else:
                btn=Button(size_hint_y=None,height=dp(72),background_normal='',background_color=base,color=(1,1,1,1),halign='left',valign='middle',padding=(dp(10),dp(4)),markup=True)
                st=s.get('start',''); en=s.get('end',''); note=s.get('note','')[:28]
                tot=calc.get('total_hours','-') if calc else '-'
                btn.text=f"[b]{row_date}[/b]\n{typ[:20]} {st}-{en} {tot} {note}"; btn.bind(size=lambda inst,*a: setattr(inst,'text_size',(inst.width-dp(20),None))); btn.bind(on_press=lambda inst,u=uid: self.open_editor(u)); self.grid.add_widget(btn)
        def fmt(m): return f"{m//60:02d}:{m%60:02d}"
        auto_norm=self.get_auto_norm_minutes(); effective=totals['worked']+totals['vacation']; saldo=effective-auto_norm; sign='+' if saldo>=0 else ''; saldo_str=f"{sign}{fmt(abs(saldo))}" if saldo!=0 else "0:00"
        ...
        if not self.select_mode: self.lbl_info.text=f'{len(month_data)} záznamov • {format_date_with_weekday.__name__}'
    ...
    def open_editor(self,uid=None):
        ...
        self.editor_screen.clear_widgets()
        
        root=BoxLayout(orientation='vertical',spacing=dp(6),padding=dp(8))
        with root.canvas.before:
            Color(*self.theme['bg'])
            bg_rect = Rectangle(size=root.size, pos=root.pos)
        root.bind(size=lambda inst, val: setattr(bg_rect, 'size', val), pos=lambda inst, val: setattr(bg_rect, 'pos', val))
        
        header_title = 'Upraviť zmenu' if not is_new else 'Pridať zmenu'
        root.add_widget(Label(text=header_title, size_hint_y=None, height=dp(30), bold=True, font_size='18sp', color=self.theme['text']))
        
        scroll=ScrollView(); form=GridLayout(cols=1,spacing=dp(8),size_hint_y=None,padding=dp(4)); form.bind(minimum_height=form.setter('height'))
        ...
        def get_unique_list(key):
            ...
            for x in all_data:
                v = x.get(key)
                if v and v.strip() and v != '-': vals.add(v.strip())
            return sorted(list(vals))

        hist_tf = get_unique_list('train_first')
        hist_tl = get_unique_list('train_last')
        hist_rf = get_unique_list('route_from')
        hist_rt = get_unique_list('route_to')

        def build_autocomplete_field(hint, text_val, history_list):
            box = BoxLayout(spacing=0)
            ti = TextInput(text=text_val, hint_text=hint, multiline=False, background_color=self.theme['input_bg'], foreground_color=self.theme['input_fg'])
            if history_list:
                ...
                for h in history_list[-20:]: 
                    ...
                    dd.add_widget(btn_item)
                btn.bind(on_release=dd.open)
                dd.bind(on_select=lambda instance, x: setattr(ti, 'text', x))
                box.add_widget(ti)
                box.add_widget(btn)
            else:
                box.add_widget(ti)
        ...
        row_t=BoxLayout(size_hint_y=None,height=dp(40),spacing=dp(4))
        it=TextInput(text=edit.get('turnus',''),hint_text='TD',multiline=False,size_hint_x=0.25,background_color=self.theme['input_bg'],foreground_color=self.theme['input_fg'])
        itf, box_itf = build_autocomplete_field('Vlak 1.', edit.get('train_first',''), hist_tf)
        itl, box_itl = build_autocomplete_field('Vlak posl.', edit.get('train_last',''), hist_tl)
        row_t.add_widget(it); row_t.add_widget(box_itf); row_t.add_widget(box_itl); add_row('TD + Vlaky',row_t)
        
        row_r=BoxLayout(size_hint_y=None,height=dp(40),spacing=dp(4))
        irf, box_irf = build_autocomplete_field('Z', edit.get('route_from',''), hist_rf)
        irt, box_irt = build_autocomplete_field('Do', edit.get('route_to',''), hist_rt)
        row_r.add_widget(box_irf); row_r.add_widget(box_irt); add_row('Trať Z → Do',row_r)
        ...
        def upd(*_):
            ...
            if c: lbl_prev.text=f"Odprac:{c['total_hours']} Dov:{c.get('vacation_minutes',0)//60:02d}:{(c.get('vacation_minutes',0)%60):02d} Strava:{c['meal_allowance']}€"
        ...
        bc.bind(on_press=go_back)
        
        def do_del(*_):
            if not is_new: nd=[d for d in all_data if d.get('uid')!=uid]; self.save_data(nd); self.refresh()
            go_back()
            
        def do_save(*_):
            try: datetime.datetime.strptime(inp_date.text,"%Y-%m-%d")
            except: self.show_info('Zlý dátum YYYY-MM-DD'); return
            ...
            self.save_data(sorted(all_data,key=lambda x:(x.get('date',''),x.get('start','')))); self.refresh()
            go_back()
    ...
    def open_turnus_manager(self,*a):
        ...
        def do_imp_text(*_):
            def on_data(data, path):
                try:
                    ...
                    else: self.show_info('Neplatný JSON'); return
                    self.save_turnus_file(tf); self.show_info('Turnus importovaný z textu')
                except Exception as e: self.show_info(f'Chyba: {e}')
        ...
        b_imp.bind(on_press=do_imp); b_imp_t.bind(on_press=do_imp_text); b_exp.bind(on_press=do_exp); row_ie.add_widget(b_imp); row_ie.add_widget(b_imp_t); row_ie.add_widget(b_exp); box_ie.add_widget(row_ie); box_ie.add_widget(Label(text='Ak Download prázdny, použi 📋 Z textu',size_hint_y=None,height=dp(36),font_size='10sp',color=self.theme['subtext'])); form.add_widget(box_ie)
        ...
        def gen(*_):
            ...
            for day in range(1,dim+1):
                ...
                if m: sh={"uid":str(uuid.uuid4()),"date":ds,"shift_type":m.get('shift_type','Štandardný výkon'),"train_first":m.get('train_first',''),"route_from":m.get('route_from',''),"train_last":m.get('train_last',''),"route_to":m.get('route_to',''),"turnus":str(ctd),"start":m.get('start',''),"end":m.get('end',''),"pc_start":m.get('pc_start',''),"pc_end":m.get('pc_end',''),"plan":m.get('plan',''),"pnp_start":m.get('pnp_start',''),"pnp_end":m.get('pnp_end',''),"meal_override":m.get('meal_override',''),"other_premiums":m.get('other_premiums',''),"note":""}
                    ...
                    if typ=='Voľno': sh={"uid":str(uuid.uuid4()),"date":ds,"shift_type":"T.V. (Turnus voľno)","train_first":"T.V.","route_from":"","train_last":"T.V.","route_to":"","turnus":str(ctd),"start":"","end":"","pc_start":"","pc_end":"","plan":"","pnp_start":"","pnp_end":"","meal_override":"","other_premiums":"","note":typ}
                    else: s,e,p=SHIFT_TIMES[typ]; sh={"uid":str(uuid.uuid4()),"date":ds,"shift_type":"Štandardný výkon","train_first":"","route_from":"","train_last":"","route_to":"","turnus":str(ctd),"start":s,"end":e,"pc_start":"","pc_end":"","plan":p,"pnp_start":"","pnp_end":"","meal_override":"","other_premiums":"","note":typ}
                nm.append(sh)
        ...
        def do_text_import(*_):
            def on_data(data, path):
                try:
                    ...
                    self.save_data(sorted(ex,key=lambda x:x.get('date',''))); self.refresh(); self.show_info(f'Import z textu: {ad} nových')
                except Exception as e: self.show_info(f'Chyba: {e}')
            TextImportPopup(on_data).open()
    ...
    def export_ics(self, *a):
        fn = f"zsskzmeny_{self.month_key()}.ics"
        out = os.path.join(self.user_data_dir, fn)
        try:
            lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//ZSSK Zmeny//SK"]
            for d in self.load_data():
                if not d.get('date', '').startswith(self.month_key()): continue
                
                typ = d.get('shift_type', 'Zmena')
                st = d.get('start', '').strip()
                en = d.get('end', '').strip()
                dt_str = d.get('date', '').replace('-', '')
                ...
                if not st or not en or st == '-' or en == '-':
                    ...
                    try:
                        ...
                        if eh < sh:
                            td = datetime.datetime.strptime(d.get('date'), "%Y-%m-%d") + datetime.timedelta(days=1)
                            end_date_str = td.strftime("%Y%m%d")
                    except: pass
                    
                    lines.append(f"DTEND:{end_date_str}T{en_time}")
                
                lines.append(f"SUMMARY:{typ}")
                
                desc_parts = []
                if d.get('route_from') or d.get('route_to'):
                    desc_parts.append(f"Trať: {d.get('route_from','')} - {d.get('route_to','')}")
                if d.get('train_first') or d.get('train_last'):
                    desc_parts.append(f"Vlaky: {d.get('train_first','')} - {d.get('train_last','')}")
                if d.get('note'):
                    desc_parts.append(f"Poznámka: {d.get('note')}")
    ...
    def open_settings(self,*a):
        ...
        form.add_widget(Label(text=f'Norma pre {self.month_key()}: {days}×5:08={auto_str}\nDovolenka 5:08/deň znižuje normu.',size_hint_y=None,height=dp(44),font_size='11sp',color=self.theme['subtext'],halign='left'))
        
        form.add_widget(Label(text='Vstavané dáta z APK (tvoj data.json)',size_hint_y=None,height=dp(24),bold=True,color=self.theme['accent']))
        bundled=find_bundled_data()
        if bundled:
            info="\n".join([f"• {os.path.basename(p)} ({os.path.getsize(p)//1024}kB)" for p in bundled[:3]])
            form.add_widget(Label(text=f"Nájdené v APK:\n{info}",size_hint_y=None,height=dp(60),font_size='10sp',color=self.theme['text'],halign='left'))
        else:
            form.add_widget(Label(text="V APK nie je data.json - pridaj data.json vedľa main.py v GitHub a skompiluj",size_hint_y=None,height=dp(40),font_size='10sp',color=self.theme['subtext']))
        b_load_bundle=Button(text='📥 Načítať vstavané dáta z APK (prepíše aktuálne)',size_hint_y=None,height=dp(48),background_normal='',background_color=self.theme['btn_primary'],font_size='11sp',bold=True)
        ...
        b_oprav=Button(text='Oprav prázdne typy',size_hint_y=None,height=dp(42),background_normal='',background_color=self.theme['btn_oprav'],font_size='11sp')
        def do_oprav(*_):
            ...
            self.save_data(data); self.refresh(); self.show_info(f'Opravených {ch}')
        ...
        b_del_month=Button(text=f'🗑 Vymazať mesiac {self.month_key()}',size_hint_y=None,height=dp(44),background_normal='',background_color=self.theme['btn_danger'],font_size='11sp'); b_del_month.bind(on_press=lambda *x: (pop.dismiss(), self.delete_month_confirm())); form.add_widget(b_del_month)
        ...
        def del_all_confirm(*_):
            pop.dismiss(); p2=Popup(title='Vymazať všetko?',size_hint=(0.90,None),height=dp(300),separator_color=self.theme['btn_danger']); b=BoxLayout(orientation='vertical',padding=dp(14),spacing=dp(10)); b.add_widget(Label(text='Vymaže sa celá databáza zmien.\nTurnusy ostanú.',halign='center',color=self.theme['text'])); r=BoxLayout(size_hint_y=None,height=dp(48),spacing=dp(8)); b1=Button(text='Zrušiť',background_normal='',background_color=self.theme['btn_settings']); b2=Button(text='Áno',background_normal='',background_color=self.theme['btn_danger'],bold=True)
            b1.bind(on_press=p2.dismiss)
            def do_wipe(*_): self.save_data([]); self.refresh(); p2.dismiss(); self.show_info('Vymazané.')
        ...
        b_reset=Button(text='♻ Reset apky',size_hint_y=None,height=dp(48),background_normal='',background_color=(0.55,0.05,0.05,1),font_size='11sp',bold=True)
        def reset_confirm(*_):
            pop.dismiss(); p3=Popup(title='RESET',size_hint=(0.92,None),height=dp(340),separator_color=(0.55,0.05,0.05,1)); b=BoxLayout(orientation='vertical',padding=dp(14),spacing=dp(10)); b.add_widget(Label(text='Vymaže zmeny, turnusy, nastavenia.\nPokračovať?',halign='left',color=self.theme['text'],font_size='12sp')); r=BoxLayout(size_hint_y=None,height=dp(50),spacing=dp(8)); b1=Button(text='Zrušiť',background_normal='',background_color=self.theme['btn_settings']); b2=Button(text='RESET',background_normal='',background_color=(0.55,0.05,0.05,1),bold=True)
    ...
    def show_info(self,msg):
        p=Popup(title='Info',size_hint=(0.88,None),height=dp(380)); b=BoxLayout(orientation='vertical',padding=dp(12),spacing=dp(8)); b.add_widget(Label(text=msg,halign='center',color=self.theme['text'],font_size='12sp')); btn=Button(text='OK',size_hint_y=None,height=dp(42),background_normal='',background_color=self.theme['btn_settings']); btn.bind(on_press=p.dismiss); b.add_widget(btn); p.content=b; p.open()
